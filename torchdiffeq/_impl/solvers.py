...
class FixedGridODESolver_ABS(object):
    __metaclass__ = abc.ABCMeta
    ...
    def integrate(self, t):
        ...
        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            dy = self.step_func(self.func, t0, t1 - t0, y0)
            dy_abs = self.step_func_abs(self.func, t0, t1 - t0, y0)

            y1 = tuple(y0_ + dy_ for y0_, dy_ in zip(y0, dy))
            y1_abs = tuple(y0_ + dy_ for y0_, dy_ in zip(y0_abs, dy_abs))
            ...
            while j < len(t) and t1 >= t[j]:
                # solution.append(self._linear_interp(t0, t1, y0, y1, t[j]))
                solution_abs.append(self._linear_interp(t0, t1, y0_abs, y1_abs, t[j]))
                j += 1
            
        # return tuple(map(torch.stack, tuple(zip(*solution))))
        return tuple(map(torch.stack, tuple(zip(*solution_abs))))
