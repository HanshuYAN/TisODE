...
# def evaluate_loss(model, dataset_loader, criterion, criterion_ss):
#     loss_dl = []
#     for _, batch in enumerate(dataset_loader):
#         logits, x_ss_target, x_ss_diff = model(batch[0].cuda(args.device_ids[0]))
#         l1 = criterion(logits, batch[1].cuda(args.device_ids[0]))
#         l2 = criterion_ss(x_ss_diff, x_ss_target)
#         loss = l1 + l2*args.w_steady
#         loss_dl.append(loss.detach().cpu().numpy())
#     return np.mean(loss_dl)
    ...
    if args.isTrain:
        ...
        for epoch in range(args.start_epoch, args.end_epoch):
            ...
            for _, batch_tr in enumerate(train_loader):
                optimizer.zero_grad()
                logits, x_ss_target, x_ss_diff = model(batch_tr[0].cuda(args.device_ids[0]))
                l1 = criterion(logits, batch_tr[1].cuda(args.device_ids[0]))
                if epoch >= args.epoch_steady:
                    l2 = criterion_ss(x_ss_diff, x_ss_target)
                    loss = l1 + l2*args.w_steady
                else:
                    l2 = torch.tensor([0])
                ...
                loss.backward()
                optimizer.step()
                # epoch_loss_pred.append(l1.detach().cpu().numpy())
                # epoch_loss_steady.append(l2.detach().cpu().numpy())
            # evaluation
            model.eval()
            with torch.no_grad():
